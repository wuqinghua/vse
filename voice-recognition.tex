...
\begin{itemize}
	\item 95\% of voice recognition flows contain no more than 4 data packets, which fit into the initial congestion window of the client.  Ideally, the voice data can be transmitted in 1 RTT.
	\item For flows with smaller RTT (less than 200ms in WiFi), the finish time is proportional to the RTT value. However, flows with larger RTT (more than 200ms) experience large finish time. Flows in WiFi experience larger finish times compared to those in cellular, due to larger RTT.
	\item Reordered packets, an indication of packet loss, require time for loss recovery, increasing the finish time by 4-6 times. Even though WiFi flows have fewer reordered packets than 3G flows, they suffer more from the reordered packets.
	\item WiFi flows suffer more from timeout retransmissions than 3G flows. WiFi flows experience timeout retransmission either during connection establishment (2.6\%) or during data transfer (9.5\%). Flows with timeout retransmissions have finish times one order of magnitude larger than those without timeout retransmission.
\end{itemize}
